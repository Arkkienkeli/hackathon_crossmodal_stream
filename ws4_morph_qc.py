...
def guess_compound_key(adata, candidates=("drug", "compound", "pert_iname",
                                          "Metadata_Drug", "Metadata_compound", "broad_id",
...
def within_site_reproducibility(adata, compound_key=None, features=None,
                                n_null=20000, percentile=95, max_pairs=20):
    """Replicate-well agreement within one site."""
    rng = np.random.default_rng(RANDOM_STATE)
    key = compound_key or guess_compound_key(adata)
    ...
    if features is not None:
        cols = pd.Index(adata.var_names).get_indexer(pd.Index(features))
        X = X[:, cols[cols >= 0]]

    keep = np.isfinite(X).all(0) & (np.nanstd(X, axis=0) > 0)
    X = X[:, keep]

    X = (
        X - X.mean(1, keepdims=True)
    ) / (
        X.std(1, keepdims=True) + 1e-12
    )

    matched, n_reps = [], []

    for cmp_id in pd.unique(labels):
        rows = np.flatnonzero(labels == cmp_id)
        n_reps.append(len(rows))

        if len(rows) < 2:
            continue

        if len(rows) > max_pairs:
            rows = rng.choice(rows, max_pairs, replace=False)

        S = (X[rows] @ X[rows].T) / X.shape[1]
        iu = np.triu_indices(len(rows), k=1)

        matched.append(float(np.median(S[iu])))
    ...
    ok = (ii != jj) & (labels[ii] != labels[jj])

    null = (X[ii[ok]] * X[jj[ok]]).mean(1)
    thr = float(np.percentile(null, percentile))

    matched = np.asarray(matched)

    out = {
        "n_compounds_with_replicates": int(len(matched)),
        "median_replicates_per_compound": float(np.median(n_reps)),
        "median_within_site_r": float(np.median(matched)),
        "median_null_r": float(np.median(null)),
        f"null_p{percentile}": thr,
        "percent_replicating": float((matched > thr).mean()),
    }
    ...
    if out["median_within_site_r"] < thr:
        print(
            "  -> replicate wells of the same compound do not agree "
            "above the null; treat downstream results with caution"
        )
