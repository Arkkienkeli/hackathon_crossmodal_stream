...
def seurat_hvg_no_batch_chunked(
    adata,
    n_top_genes=2000,
    n_bins=20,
    chunk_size=1000,
    ...
    """
    Memory-safe equivalent of Scanpy's no-batch
    highly_variable_genes(..., flavor="seurat").

    Input X is already log1p-normalized.

    Instead of doing np.expm1() on the entire 542k x 62k
    sparse matrix, process cells in row chunks and accumulate
    per-gene sums and squared sums.
    """
    ...
    if not sparse.issparse(X):
        raise ValueError(
            "Expected sparse expression matrix for this "
    ...
    # Match Scanpy handling if log1p used a non-natural base.
    log_base = (
        adata.uns
        .get("log1p", {})
        .get("base", None)
    ...
    if log_base is None:
        ...
        log_multiplier = np.log(log_base)
    ...
    for start in range(
        ...
        # Only transform stored non-zero entries.
        vals = chunk.data.astype(
            np.float64,
            copy=True,
        )

        if log_multiplier != 1.0:
            vals *= log_multiplier

        # Scanpy flavor="seurat" reverses log1p here.
        np.expm1(
            vals,
            out=vals,
        )

        # CSR indices are column/gene indices.
    ...
    # --------------------------------------------------------------
    # Mean and unbiased variance, matching Scanpy convention
    # --------------------------------------------------------------
    ...
    # Small numerical negatives can occur from floating point.
    var_raw[var_raw < 0] = 0.0
    ...
    # Seurat flavor uses log dispersion and log mean.
    with np.errstate(
        divide="ignore",
        invalid="ignore",
    ):
    ...
    # --------------------------------------------------------------
    # Normalize dispersion inside mean-expression bins
    # Scanpy default: n_bins=20
    # --------------------------------------------------------------
    ...
    # Scanpy special case:
    # one gene in a bin -> normalized dispersion = 1
    one_gene_bins = (
        stats["dev"].isna()
    )
    ...
    # --------------------------------------------------------------
    # Top-N selection using same threshold principle as Scanpy
    # --------------------------------------------------------------
    ...
    if len(finite) == 0:
        raise RuntimeError(
            "No finite normalized dispersions found."
        )
    ...
    print(
        "normalized-dispersion cutoff:",
        float(cutoff),
    )

    # Populate same fields expected downstream.
    adata.var[
        "highly_variable"
    ] = hv
